...
test_that("throws error when granularities to be plotted are not specified", {
  expect_error(gran_advice(x), "argument \"gran1\" is missing, with no default")
})


test_that("throws error with just one granularity", {
  expect_error(gran_advice(x, gran2 = "hour_day"), "argument \"gran1\" is missing, with no default")
})


test_that("throws error with no hierarchy table specified for non temporal data", {
  expect_error(gran_advice(cricket_tsibble, "ball_over", "over_inning"), "Hierarchy table must be provided when class of index of the tsibble is not date-time")
})

test_that("throws error with incorrect input for granularities", {
  expect_error(gran_advice(cricket_tsibble, "balls_over", "over_inning", hierarchy_model), "lower part of granularity must be listed as an element in the hierarchy table")
})
