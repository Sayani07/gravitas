...
test_that("throws error with no hierarchy table specified for non temporal data", {
  expect_error(prob_plot(cricket_tsibble, "ball_over", "over_inning"), "Hierarchy table must be provided when class of index of the tsibble is not date-time")
})

test_that("throws error with incorrect input for granularities", {
  expect_error(prob_plot(cricket_tsibble, "balls_over", "over_inning", hierarchy_model), "lower part of granularity must be listed as an element in the hierarchy table")
})
