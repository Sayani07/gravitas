...
step1_data <-list(
  ...
  data_4 %>%
    ...
    pivot_wider(names_from = Var1,
                values_from = sim_dist,  values_fn = list(sim_dist = list))

)

# data doesn't matter after step1_data has been fed in
global_harmony_iter4 <-  smart_meter10 %>% global_threshold(harmony_tbl = harmonies, response, step1_data = step1_data, hierarchy_tbl = hierarchy_model)
