...
The `vic_elec` data from tsibbledata package is employed to have a run through of this package. The data consists of half hour demand for the state of Victoria. Suppose it is of interest to examine how the univariate series `Demand` varies with different deconstructions of time.
...
```{r example10}
tsibbledata::vic_elec %>% create_gran("day_fortnight") %>% ggplot2::ggplot(aes(x = as.factor(day_fortnight), y = Demand)) + xlab("day_fortnight") + geom_boxplot()
```
