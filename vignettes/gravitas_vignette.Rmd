...
## <h>style="color:blue;"> Get started</h>
...
smart_meter_nsw %>% search_gran(ugran = "month", filter_out = "fortnight", filter_in = "wknd_wday")
```
...
smart_meter_nsw  %>% harmony(ugran = "month", filter_out = c("fortnight", "hhour"))
```


### Explore probability distribution across bivariate temporal granularities with `granplot()`

Now, we want to view distribution of the measured variable `general_supply_kwh` across these bivariate granularities through boxplots using `granplot`.

```{r example6, echo=TRUE}
smart_meter_nsw  %>% granplot("hour_day", "day_week", "general_supply_kwh", plot_type = "boxplot")
```
Now, we want to view distribution of `general_supply_kwh` across the next set of harmonies - hour_day and day_month and let the package decide 
the plot type that is best suitable for the number of levels in hour_day and day_month.

```{r example7, echo=TRUE}
smart_meter_nsw  %>% granplot("hour_day", "day_week", "general_supply_kwh")
```
...
smart_meter_nsw  %>% granplot("day_week", "hour_day", "general_supply_kwh", plot_type = "violin") + ggplot2::scale_x_discrete(breaks = seq(1, 24, 3))
```
...
smart_meter_nsw  %>% granplot(gran1 = "hour_week", gran2 = "day_week", "general_supply_kwh", plot_type = "violin") + ggplot2::scale_x_discrete(breaks = seq(1, 24, 2))
```
...
smart_meter_nsw  %>% create_gran("day_fortnight") %>% ggplot2::ggplot(aes(x = as.factor(day_fortnight), y = general_supply_kwh)) + xlab("day_fortnight") + geom_boxplot()
```
