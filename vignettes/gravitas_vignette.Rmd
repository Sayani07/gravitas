...
In this vignette, we describe the tools in the package gravitas for systematically exploring large quantities of temporal data across different time deconstructions by visualizing the probability distributions. 


# Computation of time granularities: 
...
#smart_meter10 <- smart_meter10 %>% as_tsibble() %>% ungroup()
```
...
smart_meter10 %>% search_gran(highest_unit = "month", filter_out = "fortnight")
```
...
smart_meter10 %>% is_harmony(gran1 = "day_month", gran2 = "hhour_week")
```
...
smart_meter10  %>% prob_plot("day_week",
                              "hour_day",
                              response  = "general_supply_kwh",
